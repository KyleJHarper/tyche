 ...
 */
void options__process(int argc, char **argv, Options *opts) {
  ...
  int c = 0;
  opterr = 0;
  while ((c = getopt(argc, argv, "b:d:f:hm:n:p:r:t:w:")) != -1) {
    switch (c) {
      case 'b':
      ...
      case '?':
        options__show_help();
        if (optopt == 'b' || optopt == 'd' || optopt == 'f' || optopt == 'm' || optopt == 'n' || optopt == 'p' || optopt == 'r' || optopt == 'w')
...
void options__show_help() {
  ...
  fprintf(stderr, "\n");
  fprintf(stderr, "  Usage: tyche <-p pages_directory> <-m memory_size> [-bdfhnrtw]\n");
